...
export default function SuperAdminPanel() {
  ...
  // "Just in case" repair: re-links any recipe ingredient (Product.baseRecipe/
  // sizes[].recipe/addOns[].recipe, ModifierGroup.options[].recipe) whose
  // invId snapshot no longer points to a real Inventory doc - e.g. right
  // after Purge Data + recreating inventory. Safe to run anytime, including
  // when there's nothing to fix.
  const handleRewireRecipes = async () => {
    setRewireModal(m => ({ ...m, busy: true, error: '' }));
    try {
      const res = await apiFetch('/api/admin/rewire-recipes', { method: 'POST' });
      const d = await res.json();
      if (d.success) setRewireModal(m => ({ ...m, busy: false, result: d }));
      else setRewireModal(m => ({ ...m, busy: false, error: d.error || 'Rewire failed.' }));
    } catch { setRewireModal(m => ({ ...m, busy: false, error: 'Network error.' })); }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => !rewireModal.busy && setRewireModal({ open: false, busy: false, error: '', result: null })}>
          <div className="bg-sidebar-bg border border-white/10 rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            ...
            {rewireModal.result ? (
              ...
              <div className="p-6 space-y-4">
                <p className="text-fg/70 text-sm">
                  Scans every product's recipe (base recipe, size variants, add-ons) and every modifier group's
                  options for an ingredient whose linked inventory item no longer exists, and re-links it to a
