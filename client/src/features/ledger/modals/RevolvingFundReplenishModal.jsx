          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
            <div>
              ...
              <p className="text-fg/65 text-xs font-bold uppercase tracking-widest mt-0.5">
                {rfActiveFund.name} · Shortfall: <span className="text-brand-text">₱{(rfActiveFund.initialAmount - rfActiveFund.currentBalance).toFixed(2)}</span>
              </p>
            </div>
            <button onClick={() => setRfReplModal(false)} className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 text-fg/75 flex items-center justify-center transition"><X size={16}/></button>
          </div>
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 custom-scrollbar">
            <div>
              <label className="text-[10px] text-fg/70 font-bold uppercase block mb-1">Amount to Add (₱)</label>
              ...
              <p className="text-fg/65 text-[10px] mt-1">Leave blank to auto-fill the full shortfall (₱{(rfActiveFund.initialAmount - rfActiveFund.currentBalance).toFixed(2)}).</p>
            </div>
            <div>
              <label className="text-[10px] text-fg/70 font-bold uppercase block mb-1">Get funds from *</label>
              ...
              <p className="text-fg/65 text-[10px] mt-1">This account is credited (reduced) when the petty cash float is funded.</p>
            </div>
            <div>
