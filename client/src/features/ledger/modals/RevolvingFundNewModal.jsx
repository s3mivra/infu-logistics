      ...
      <div className="fixed inset-0 z-[9998] bg-black/85 flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-sm" onClick={e => { if (e.target === e.currentTarget) setRfNewModal(false); }}>
        <div className="bg-surface border border-white/10 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md shadow-elev-3 flex flex-col max-h-[92vh] overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
            ...
            <button onClick={() => setRfNewModal(false)} className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 text-fg/75 flex items-center justify-center transition"><X size={16}/></button>
          </div>
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 custom-scrollbar">
            ...
            <div>
              ...
              <p className="text-fg/65 text-[10px] mt-1">This is the fixed float amount. The source account below will be reduced by this amount in the journal.</p>
            </div>
            <div>
              <label className="text-[10px] text-fg/70 font-bold uppercase block mb-1">Paid From *</label>
              ...
              <p className="text-fg/65 text-[10px] mt-1">Where the float comes from - this account is credited (reduced) in the opening journal entry.</p>
            </div>
            <div>
