      ...
      <div className="bg-surface border border-red-500/25 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md shadow-elev-3 flex flex-col animate-scale-in">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-red-500/15 text-danger flex items-center justify-center shrink-0"><AlertTriangle size={17}/></div>
            <div>
              <h2 className="text-fg font-black text-lg">Bounce Check</h2>
              <p className="text-fg/70 text-xs mt-0.5">#{c.checkNumber}{c.checkBank ? ` · ${c.checkBank}` : ''}</p>
            </div>
          </div>
          <button onClick={() => setBounceTarget(null)} className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 text-fg/75 flex items-center justify-center transition" aria-label="Close"><X size={16}/></button>
        </div>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3">
            <p className="text-[11px] text-fg/70 leading-relaxed">
              ...
              <span className="text-danger font-black tabular-nums">{peso(c.amount)}</span> again on{' '}
          ...
          <div>
            <label className="text-[10px] text-fg/70 font-bold uppercase block mb-1">Date Returned</label>
            ...
            <p className="text-[9px] text-fg/65 mt-1">The reversing journal entry is booked on this date.</p>
          </div>
        </div>
