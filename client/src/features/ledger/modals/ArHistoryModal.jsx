      ...
      <div className="bg-surface border border-white/10 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-2xl shadow-elev-3 flex flex-col max-h-[92vh] animate-scale-in">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <div>
            ...
            <p className="text-fg/70 text-xs mt-0.5">{order.orderNumber} · {order.customerName || 'No name'}</p>
          </div>
          <button onClick={() => setArHistory(null)} className="w-10 h-10 rounded-xl bg-white/5 hover:bg-white/10 text-fg/75 flex items-center justify-center transition" aria-label="Close"><X size={16}/></button>
        </div>

        <div className="px-5 py-4 space-y-4 overflow-y-auto">
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white/5 rounded-xl p-3 border border-white/10">
              <p className="text-fg/70 text-[10px] font-bold uppercase">Invoiced</p>
              <p className="text-xl text-fg font-black tabular-nums">{peso(order.total)}</p>
            </div>
            <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20">
              <p className="text-fg/70 text-[10px] font-bold uppercase">Collected</p>
              <p className="text-xl text-success font-black tabular-nums">{peso(totalPaid)}</p>
            </div>
          ...
          {arHistoryLoading ? (
            <div className="py-12 text-center text-fg/70 font-bold uppercase tracking-widest text-xs">Loading…</div>
          ) : !payments?.length ? (
            <div className="py-12 text-center text-fg/70 font-bold uppercase tracking-widest text-xs">No collections recorded yet</div>
          ) : (
                  ...
                  {payments.map((p, i) => (
                    <tr key={p._id || i} className="border-b border-white/5">
                      <td className="py-2.5 text-fg/70 text-xs">
                        ...
                        {p.collectedBy && <span className="block text-[9px] text-fg/70">by {p.collectedBy}</span>}
                      </td>
                      <td className="py-2.5 text-fg/70 text-xs">
                        {day(p.depositDate)}
                      ...
                      <td className={`py-2.5 text-right tabular-nums font-black ${p.balanceAfter > 0.01 ? 'text-fg' : 'text-fg/65'}`}>{peso(p.balanceAfter)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
